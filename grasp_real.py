...
def grasp(data_input,cloud_,point_left_up,point_right_bottom):
    ...
    # cam_pose_calibration = ur_robot.read_cam_external_parament(cam_pose_calibration_file)
    cam_pose_calibration_file = 'real/cam_pose/calibration.yaml'
    cam2end = tf_test.get_cam2end(cam_pose_calibration_file)
    print("--------cam2end--------")
    print(cam2end)
    ...
    grasp2end = np.dot(cam2end, grasp2camera)
    print("--------grasp2end--------")
    print(grasp2end)
    # grasp2robot = ur_robot.tcp_transform_base(grasp2end)
    grasp2robot = tf_test.tcp_transform_base(grasp2end)
    print("--------grasp2robot--------")
    print(grasp2robot)
    ...
    success = True
    # success = ur_robot.grasp_ros(t_grasp2robot, rpy_grasp2robot, open_size=width)  # grasp_ROS
    success = ur_robot.grasp(t_grasp2robot, rpy_grasp2robot, open_size=width)  # grasp_socket
    return 0
